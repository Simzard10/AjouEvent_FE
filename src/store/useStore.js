...
const useStore = create((set) => ({
  ...
  markTopicAsRead: async (topic) => set(async (state) => {
    ...
    if (allTopicsRead && !state.isTopicTabRead) {
      try {
        ...
        console.error("Error updating topic read status on the server:", error);
      }
    }
  }),

  // 특정 토픽을 읽음 상태로 변경하고 전체 읽음 상태를 갱신하는 함수
  markKeywordAsRead: async (keyword) => set(async (state) => {
    const updatedItems = state.subscribedKeywords.map((item) =>
      item.encodedKeyword === keyword.encodedKeyword ? { ...item, isRead: true } : item
    );
    ...
    // 모든 항목이 읽음 상태일 경우 서버에 읽음 상태 업데이트
    if (allKeywordsRead && !state.isKeywordTabRead) {  // 중복 API 호출 방지
      try {
        await requestWithAccessToken("post", `${process.env.REACT_APP_BE_URL}/api/event/updateKeywordTabRead`);
    } catch (error) {
      console.error("Error updating keyword read status on the server:", error);
    }
  }
  }),
  ...
  fetchSubscribedTopics: async () => {
    try {
      const response = await requestWithAccessToken("get", `${process.env.REACT_APP_BE_URL}/api/topic/subscriptions`);
  ...
  fetchSubscribedKeywords: async () => {
    try {
      const response = await requestWithAccessToken("get", `${process.env.REACT_APP_BE_URL}/api/keyword/userKeywords`);
  ...
  subscribeToTopic: async (topic) => {
    try {
      await requestWithAccessToken("post", `${process.env.REACT_APP_BE_URL}/api/topic/subscribe`, { topic });
      set((state) => ({
        topics: state.topics.map(t => t.englishTopic === topic ? { ...t, subscribed: true } : t)
      }));
    } catch (error) {
  ...
  unsubscribeFromTopic: async (topic) => {
    try {
      await requestWithAccessToken("post", `${process.env.REACT_APP_BE_URL}/api/topic/unsubscribe`, { topic });
      set((state) => ({
        topics: state.topics.map(t => t.englishTopic === topic ? { ...t, subscribed: false } : t)
      }));
    } catch (error) {
